...
Users bring their own API key — zero cost to host
"""
...
.iq-key-card{
  ...
  padding:28px 32px;max-width:560px;margin:0 auto;
}
.iq-provider-pill{
...
.iq-provider-pill:hover{border-color:var(--accent);color:var(--text);}
...
PROVIDERS = {
    "groq":   {"name":"Groq",   "icon":"⚡", "color":"#F97316", "model":"llama-3.3-70b-versatile",
               "prefix":"gsk_",  "url":"https://console.groq.com/keys",
               "hint":"Starts with gsk_  ·  Free tier available"},
    "openai": {"name":"OpenAI", "icon":"🤖", "color":"#10A37F", "model":"gpt-4o-mini",
...
def call_ai(system: str, user: str) -> str:
    ...
    try:
        ...
        if "auth" in err.lower() or "api_key" in err.lower() or "invalid" in err.lower():
            ...
            return f"⚠️ Invalid API key. Please check your key in the sidebar.\n\nError: {err}"
        return f"⚠️ API error: {err}"

def verify_key(key: str, provider: str) -> tuple[bool, str]:
...
def run_sentiment_analysis(df):
    ...
    if text_cols:
        ...
        sys_p = """Analyze the reviews and return ONLY valid JSON with no explanation:
...
"themes_positive":["t1","t2","t3"],"themes_negative":["t1","t2","t3"],"summary":"one sentence"}"""
        ...
        except: res["distribution"]={"Positive":0,"Neutral":0,"Negative":0}

        label_raw = call_ai(
            'Label each review as Positive, Neutral, or Negative. Return ONLY a JSON array.',
        ...
        if mx<=5: pos=(scores>=4).sum(); neu=((scores>=3)&(scores<4)).sum(); neg=(scores<3).sum()
        else:     pos=(scores>=7).sum(); neu=((scores>=5)&(scores<7)).sum(); neg=(scores<5).sum()
...
def render_section_chart(df, title):
    ...
    if any(w in tl for w in ["trend","time","growth","monthly"]):
        if date_col:
            try: st.line_chart(df.set_index(date_col)[num_cols[0]],use_container_width=True)
            except: pass
    elif any(w in tl for w in ["finding","overview","performance","sales","revenue"]):
        if cat_cols:
            st.bar_chart(df.groupby(cat_cols[0])[num_cols[0]].sum().reset_index().set_index(cat_cols[0]),
                         use_container_width=True)
    elif any(w in tl for w in ["risk","return","loss"]):
        risk=[c for c in num_cols if any(k in c.lower() for k in ['return','loss','risk'])]
        col=risk[0] if risk else num_cols[-1]
        if cat_cols:
            st.bar_chart(df.groupby(cat_cols[0])[col].sum().reset_index().set_index(cat_cols[0]),
                         use_container_width=True)
...
def generate_markdown_export(report_text, title, company="InsightIQ"):
    ...
    for line in lines:
        ...
        if ls.startswith('## '): continue
            ...
            sec=ls[4:]; icon=ICONS2.get(sec.lower(),'◈')
            md.append(f"\n## {icon} {sec}\n")
        else: md.append(ls)
...
def generate_word_export(report_text, title, company="InsightIQ"):
    ...
    SEC_RGB={
        'data overview':RGBColor(0x25,0x63,0xEB),'key findings':RGBColor(0x7C,0x3A,0xED),
        'trend analysis':RGBColor(0x08,0x91,0xB2),'risk signals':RGBColor(0xDC,0x26,0x26),
        'strategic recommendations':RGBColor(0x05,0x96,0x69),
        'executive takeaway':RGBColor(0xD9,0x77,0x06),'recommendations':RGBColor(0x05,0x96,0x69),
    }
    ICONS3={'data overview':'📊 ','key findings':'🔍 ','trend analysis':'📈 ',
    ...
    def flush(sec_title,body_lines):
        ...
        while i<len(body_lines):
            ...
            if nm:
                ...
                nr=p.add_run(f"{nm.group(1)}.  "); nr.font.size=Pt(10)
                nr.font.bold=True; nr.font.color.rgb=color
            ...
            if line.startswith(('- ','* ')):
                ...
                tr=p.add_run(re.sub(r'\*\*(.+?)\*\*',r'\1',re.sub(r'[^\x00-\x7F]+','',line[2:])))
                tr.font.size=Pt(10); tr.font.color.rgb=TEXT2
                i+=1; continue
            p=doc.add_paragraph(); shading(p,'F8FAFC')
            p.paragraph_format.left_indent=Cm(0.5)
            p.paragraph_format.space_after=Pt(3)
            tr=p.add_run(re.sub(r'\*\*(.+?)\*\*',r'\1',re.sub(r'[^\x00-\x7F]+','',line)))
            tr.font.size=Pt(10); tr.font.color.rgb=TEXT2
            i+=1
...
def generate_pdf_export(report_text, title, company="InsightIQ"):
    ...
    SEC_C={'data overview':colors.HexColor('#2563EB'),'key findings':colors.HexColor('#7C3AED'),
           'trend analysis':colors.HexColor('#0891B2'),'risk signals':colors.HexColor('#DC2626'),
           'strategic recommendations':colors.HexColor('#059669'),
           'executive takeaway':colors.HexColor('#D97706'),'recommendations':colors.HexColor('#059669')}
    ...
    class HFC(rl_canvas.Canvas):
        ...
        def _draw(self,total):
            ...
            if pg==1:
                ...
                self.circle(W-28*mm,H-42*mm,28*mm,fill=1,stroke=0)
                self.setFillAlpha(1)
            else:
    ...
    def _block(sec_title,body_lines):
        key=sec_title.lower().strip()
        color=SEC_C.get(key,C_ACCENT); num=SEC_N.get(key,'◈')
        ...
        while i<len(body_lines):
            ...
            if nm:
                ...
                while i<len(body_lines):
                    sub=body_lines[i].strip()
                    if sub.startswith(('- ','* ')):
                        blk.append(Paragraph(f"<font color='#94A3B8'>  —</font>  {_clean(sub[2:])}",s_sb))
                        i+=1
                    else: break
                continue
            if line.startswith(('- ','* ')):
...
with st.sidebar:
    ...
    if st.session_state.api_key:
        ...
        if p:
            ...
            st.markdown("""
            <div class="provider-status" style="border-color:rgba(239,68,68,.3)">
    ...
    <div style="position:fixed;bottom:0;left:0;width:268px;
      ...
      <div style="font-size:11px;color:#1E3A5F;line-height:1.8">
        ...
        <span style="color:#3B82F6">Groq</span> ·
...
if not has_key:
    ...
    for i,(pid,info) in enumerate(provider_list):
        with cols[i]:
            st.markdown(f"""
    ...
    # How to use steps
    st.markdown("""
    <div style="max-width:640px;margin:36px auto 0;padding:0 20px">
      <div class="iq-card">
        ...
        <div style="display:flex;flex-direction:column;gap:12px">
          <div style="display:flex;align-items:flex-start;gap:12px">
            <div style="width:24px;height:24px;background:linear-gradient(135deg,#3B82F6,#6366F1);
            ...
            <div>
              ...
              <div style="font-size:12px;color:#64748B;margin-top:2px">
                Click any provider above — Groq is fastest and completely free
              </div>
            </div>
          </div>
          <div style="display:flex;align-items:flex-start;gap:12px">
            ...
            <div>
              ...
              <div style="font-size:12px;color:#64748B;margin-top:2px">
                The app auto-detects your provider (Groq / OpenAI / Gemini)
              </div>
            </div>
          </div>
          <div style="display:flex;align-items:flex-start;gap:12px">
            ...
            <div>
              <div style="font-size:13px;font-weight:600;color:#CBD5E1">Upload your data & analyse</div>
              <div style="font-size:12px;color:#64748B;margin-top:2px">
                CSV, Excel, or try the built-in sample datasets
              </div>
            </div>
          </div>
        </div>
    ...
    # Use cases
    st.markdown("""
    <div style="max-width:860px;margin:32px auto 0;padding:0 20px">
    ...
    if df is None:
        ...
        <div class="fade-up" style="max-width:640px;margin:48px auto 0;text-align:center;padding:0 20px">
          ...
          <p style="font-size:16px;color:#64748B;margin-bottom:32px">
            CSV or Excel · Sales data · Customer reviews · Any business dataset
          </p>
        </div>
        """, unsafe_allow_html=True)

        c_l,c_c,c_r=st.columns([1,2.2,1])
        with c_c:
        ...
        with top_l:
            provider_badge = (f'<span class="iq-badge iq-badge-blue" style="margin-right:6px">'
                              f'{prov_info.get("icon","◈")} {prov_info.get("name","AI")}</span>'
                              if prov_info else '')
            sent_badge = '<span class="iq-badge iq-badge-purple">💬 Sentiment Ready</span>' if has_sentiment else ''
            pro_badge  = '<span class="iq-badge iq-badge-gold" style="margin-left:6px">✦ Pro</span>' if is_premium else ''
            st.markdown(f"""
            <div class="fade-in" style="padding:20px 0 4px">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px">
                ...
                <span class="iq-badge iq-badge-green">● Live</span>
                {provider_badge}{sent_badge}{pro_badge}
              </div>
        ...
        if has_sentiment:
            with all_tabs[tab_idx]:
                ...
                if st.session_state.sentiment_cache:
                    res=st.session_state.sentiment_cache; dist=res["distribution"]
                    ...
                    sb.metric("😐 Neutral", f"{neu_p}%",f"{dist.get('Neutral', 0)} reviews")
                    ...
                    if dist: st.bar_chart(pd.DataFrame({"Count":dist}),use_container_width=True)
                    ...
                    if res["recommendations"]:
                        ...
                        st.markdown(f'<div class="iq-card iq-card-glow" style="font-size:14px;color:var(--text2);line-height:1.8">{res["recommendations"].replace(chr(10),"<br>")}</div>', unsafe_allow_html=True)
        ...
        with all_tabs[tab_idx]:
            ...
            sc=st.columns(4)
            for i,p in enumerate(["What trends do you see?","Which segment performs best?",
                                   "What should I improve?","Top 3 risks in my data?"]):
                if sc[i].button(p,key=f"p{i}",use_container_width=True):
                    st.session_state["pending_input"]=p
            st.markdown("<div style='height:6px'></div>", unsafe_allow_html=True)
            ic,bc=st.columns([5,1])
            with ic:
                typed=st.text_input("msg",placeholder="Ask anything…",
...
Answer precisely. Use bullet points. Cite actual numbers."""
            ...
            if not st.session_state.messages:
                ...
                st.markdown("<div style='height:10px'></div>", unsafe_allow_html=True)
                if st.button("Clear chat",key="clr"): st.session_state.messages=[]; st.rerun()

        # ── REPORTS ───────────────────────────────────────────────────────────
        with all_tabs[tab_idx]:
            tab_idx+=1
            st.markdown('<div class="iq-section">Report Generator</div>', unsafe_allow_html=True)
...
Deliver 5 specific recommendations. Each: **Bold Title**, Problem, Action, Expected Impact, Timeline."""
                    ...
                    st.markdown("""<div class="iq-locked-wrap">
                      ...
                      <div class="iq-lock-overlay"><span style="font-size:20px">🔒</span>
